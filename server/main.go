		...
		go func(conn net.Conn) {
			...
			defer func() {
				if closeErr := conn.Close(); closeErr != nil {
					log.Printf("failed to close incoming connection: %v", closeErr)
				}
			}() // graceful shutdown
			var err error = nil
			...
			context.AfterFunc(ctx2, func() {
				if err := conn.SetDeadline(time.Now()); err != nil {
					log.Printf("failed to set incoming connection deadline: %v", err)
				}
				if err := serverConn.SetDeadline(time.Now()); err != nil {
					log.Printf("failed to set outgoing connection deadline: %v", err)
				}
			})
			go func() {
				defer wg.Done()
				...
				for {
					...
					if err := serverConn.SetWriteDeadline(time.Now().Add(time.Minute * 30)); err != nil {
						log.Printf("Failed: %s", err)
						return
					}
			...
			go func() {
				...
				for {
					...
					if err := serverConn.SetReadDeadline(time.Now().Add(time.Minute * 30)); err != nil {
						log.Printf("Failed: %s", err)
						return
					}
					...
					if err := conn.SetWriteDeadline(time.Now().Add(time.Minute * 30)); err != nil {
						log.Printf("Failed: %s", err)
						return
					}
